...
write_xlsx(overlapIgg, here::here("01_raw-data", "CombinedMSD_Neut_Smartseq_OverlappingCITESeq.xlsx"))

#write xlsx of just merged MSD data
msdOnly <- overlapIgg %>% filter(!str_detect(CELL, "COV"), !is.na(donor_id)) %>%
              select("CELL", "SampleSourcePlateID","donor_id", "Donor Plate #", "CorrectedBoost", "Infection Status", "MSD Date", "RATPIg Well",
                     "Prototype", "Omicron BA.1.old", "Omicron BA.1.new", "Omicron BA.1", "Omicron BA.2", "Omicron BA 45", "Omicron XBB", "JN1", "KP.2", "BindingPopulation",
                     "D614G", "BA.1", "XBB.1.5", "JN.1", "NeutralizingPopulations",  "ValitaTiter IgG (µg/mL)", "IgGQuant")

write_xlsx(msdOnly, here::here("01_raw-data", "MSD_Neut_IgGDataCombined_NoCiteseq.xlsx"))
#####