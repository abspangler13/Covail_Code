...
finalFlow <- intermediateFlow %>%
  select(`Subject ID`, `Specimen ID`, primary_vax_series, infect_baseline, pre_study_booster, Infection, EarlyInfection, infect_flag, oosboost_flag, Timepoint, Booster, Immunogen, Platform, Company, Stage, Arm, Dataset,
         `Lymphocytes | Count`, `Lymphocytes | Freq. of Total`, `Lymphocytes/Single Cells/CD19 | Count`, `Lymphocytes/Single Cells/CD19 | Freq. of Single Cells`,
...
write.csv(finalFlow, here::here("01_raw-data", "FlowData", "FinalizedDatasets", "Unfiltered_COVAILFlowDataset_250318.csv"))
...
#save
write_xlsx(finalFlow2, here::here("04_Analysis", "data_objects", "figure_testing", "Infected_at_Baseline_flowdata_uninfectedkept.xlsx"))

#only keep donors infected at baseline
flow3 <- flow3 %>% filter(infect_baseline == "Y")

write_xlsx(finalFlow2, here::here("04_Analysis", "data_objects", "figure_testing", "Infected_at_Baseline_flowdata.xlsx"))
