...
ggsave(filename = here::here("04_Analysis", "plots", "paperfigures", "Figure 1", "PfizerVsModerna_lineplot_tabled.png"), width = 7, height=3.5, units = "in", dpi = 900)
ggsave(filename = here::here("04_Analysis", "plots", "paperfigures", "Figure 1", "PfizerVsModerna_lineplot_tabled.svg"), width = 7, height=3.5)
dev.off()
...
#####
#instead of FC, do %igg difference
#make a delimiter for manufacturer
stats <- flow %>%
  group_by(Immunogen, Company, Timepoint) %>%
  mutate(Immunogen = factor(Immunogen, levels = c("Prototype", "Prototype/Beta", "Beta", "Prototype/BA.1", "Omicron BA.1", "Beta/BA.1", "Delta/BA.1"))) %>%
...
plot_grid(p1, p2, ncol = 1, align = "hv", rel_heights = c(1, 0.8))
ggsave(filename = here::here("04_Analysis", "plots", "paperfigures", "Figure 1", "PfizerVsModerna_lineplot_tabled_percentigg.png"), width = 7, height=3.5, units = "in", dpi = 900)
ggsave(filename = here::here("04_Analysis", "plots", "paperfigures", "Figure 1", "PfizerVsModerna_lineplot_tabled_percentigg.svg"), width = 7, height=3.5)
dev.off()
...
#####
#make logged version of figure 1d
stats <- flow %>%
  group_by(Immunogen, Company, Timepoint) %>%
  mutate(Immunogen = factor(Immunogen, levels = c("Prototype", "Prototype/Beta", "Beta", "Prototype/BA.1", "Omicron BA.1", "Beta/BA.1", "Delta/BA.1"))) %>%
  summarize(length = n(),
            mean = mean(log10(TotalRBD)),
            sd = sd(log10(TotalRBD)))
