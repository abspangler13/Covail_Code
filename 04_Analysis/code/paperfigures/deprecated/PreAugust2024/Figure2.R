...
ggplot(stats, aes(x = `Time point Guess`, y=mean, fill = Booster))+
  ...
  theme(title = element_text(size = 7),
        ...
        legend.position = "none",
        axis.line = element_line(size = 0.3))
ggsave(filename = here::here("04_Analysis", "plots", "paperfigures", "Figure 2", "Figure2h_FoldCross_ProtoBeta.png"),width = 1.9, height = 3, units = "in", device = "png", dpi = 600)
dev.off()
#####

#####
#Figure j: fold change for proto-specific population
#NOTE THAT THIS HAS MOVED TO SUPP
#stage 1
# stats <- flow %>% filter(Booster %in% c("Prototype mRNA", "Prototype + BA.1 mRNA", "Omicron BA.1 mRNA") & infect_flag == "0") %>% group_by(Booster, Immunogen, `Subject ID`) %>%
#   arrange(`Time point Guess`) %>%
...
#         legend.position = "none",
#         axis.line = element_line(size = 0.3))
# ggsave(filename = here::here("04_Analysis", "plots", "paperfigures", "Figure 2", "Figure2f_FoldProtoSp_ProtoNotBeta.png"),width = 1.9, height = 3, units = "in", device = "png", dpi = 600)
# dev.off()
#####

####Make a barplot of probe specific pops
#####
#Let's try a population bar chart over time
#stage 1 comparison first
#calculate proportion of all prototype-omicron positive populations
flow$`Proto+BA.1+` <- flow$ProtoOmi / flow$TotalRBD
flow$`Proto+BA.1-` <- flow$ProtoNotOmicron / flow$TotalRBD
flow$`Proto-BA.1+` <- flow$OmicronNotPrototype / flow$TotalRBD
flow$`Proto-BA.1-` <- (flow$`Beta+` + flow$`Beta+/XBB+`) / flow$TotalRBD

table(flow$`Proto+BA.1+` + flow$`Proto+BA.1-` + flow$`Proto-BA.1+` + flow$`Proto-BA.1-`)
#should return all values = 1

#make a pivoted table
stats <- flow %>% filter(Booster %in% c("Prototype mRNA", "Omicron BA.1 mRNA", "Prototype + BA.1 mRNA")) %>%
  select(`Subject ID`, Booster, `Time point Guess`, `Proto-BA.1-`, `Proto-BA.1+`, `Proto+BA.1-`, `Proto+BA.1+`) %>%
  pivot_longer(!c(`Subject ID`, Booster, `Time point Guess`), names_to = "Population", values_to = "Proportion") %>%
  group_by(Booster, `Time point Guess`, Population) %>%
...
ggplot(stats) +
  geom_bar(aes(x=`Time point Guess`, y=mean, fill = Population), stat="identity", position="stack", color="black", linewidth = 0.3) +
  geom_errorbar(aes(x=`Time point Guess`, ymin=1-cumusum-se, ymax= ifelse(1-cumusum+se >= 1, 1, 1-cumusum+se)), width=0.2, alpha=1) +
  facet_grid(cols = vars(Booster), labeller = label_wrap_gen(15))+
...
ggsave(filename = here::here("04_Analysis", "plots", "paperfigures", "Figure 2", "Figure2_ProbePositivePopulationsOverTime_ProtoOmi.png"),width = 3.3, height = 2.4, units = "in", device = "png", dpi = 600)
dev.off()
#####
