...
######figure 1c: comparing total RBD+ over time among uninfected donors
stats <- flow %>% filter(infect_flag == "0") %>%
          select(Treatment, `Subject ID`, probeset, TotalRBD, `Time point Guess`) %>%
          pivot_wider(names_from = `Time point Guess`, values_from = TotalRBD) %>%
          mutate(AnyNAs = rowSums(is.na(.)) >= 1)

write_xlsx(stats, here::here("04_Analysis","plots","paperfigures", "stats", "Figure1", "Figure1c_TotalRBDOverTime.xlsx"))
#####

####Figure 1d: Fold change in total RBD for Beta comparison
#total RBD
placeholder <-  stats %>% filter(infect_flag == "0") %>%
  select(Booster, `Subject ID`,probeset, FoldTotalRBD, `Time point Guess`) %>%
  pivot_wider(names_from = `Time point Guess`, values_from = FoldTotalRBD)

#write sheet
write_xlsx(placeholder, path= here::here("04_Analysis","plots","paperfigures", "stats", "Figure1", "Figure1_FoldChange_TotalRBD.xlsx"))
#####




#####figure S1(idk): comparing how well things cross out to later variants
...
write_xlsx(stats, here::here("04_Analysis","plots","paperfigures", "stats", "FigureS1", "FigureS1_Stages23_RatioProtoBA1ToProtoNotBA1_OverTime.xlsx"))
...
write_xlsx(stats, here::here("04_Analysis","plots","paperfigures", "stats", "FigureS1", "FigureS1_stage1_RatioProtoXBBToProtoNotXBB_OverTime.xlsx"))


#####figure 1f: same as 2b but for infected
stats <- flow %>% filter(Infection == "Y" & Booster %in% c("Prototype mRNA", "Omicron BA.1 mRNA", "Prototype + BA.1 mRNA")) %>%
  mutate(BoosterSubject = paste(Booster, `Subject ID`)) %>%
  select(Treatment, `Subject ID`,probeset, TotalRBD, `Time point Guess`) %>%
  pivot_wider(names_from = `Time point Guess`, values_from = TotalRBD) %>%
  mutate(AnyNAs = rowSums(is.na(.)) >= 1)

write_xlsx(stats, here::here("04_Analysis","plots","paperfigures", "stats", "Figure1", "Figure1f_TotalRBDOverTime_infected.xlsx"))
#####

#####
#Figure 1i: fold changes over time
# stats <- flow %>% filter(!(`Subject ID` %in% flow$`Subject ID`[flow$TotalRBD == 0])) %>% 
#   group_by(`Subject ID`) %>%
#   arrange(`Time point Guess`) %>%
#   mutate(FoldTotalRBD = TotalRBD / TotalRBD[1],
#          FoldTotalProtoBeta = ProtoBeta / ProtoBeta[1],
#          FoldTotalProtoNotBeta = ProtoNotBeta / ProtoNotBeta[1],
#          FoldTotalBetaNotProto = BetaNotProto / BetaNotProto[1],
#          FoldTotalProtoOmi = ProtoOmi / ProtoOmi[1],
#          FoldTotalProtoNotOmi = ProtoNotOmicron / ProtoNotOmicron[1],
#          FoldTotalOmiNotProto = OmicronNotPrototype / OmicronNotPrototype[1])
####
...
openxlsx::write.xlsx(stage1list, file= here::here("04_Analysis","plots","paperfigures", "stats", "Figure1", "Figure1_FoldChange_Stage1Comparisons.xlsx"))
...
#write one sheet for each fold change
openxlsx::write.xlsx(stage23list, file= here::here("04_Analysis","plots","paperfigures", "stats", "Figure1", "Figure1_FoldChange_Stages23Comparisons.xlsx"))
#####

#####
#BA.1 only
stats <- flow %>% filter(Booster %in% c("Prototype mRNA", "Omicron BA.1 mRNA", "Prototype + BA.1 mRNA") & infect_flag == "0") %>%
  mutate(BoosterSubject = paste(Booster, `Subject ID`)) %>%
  ...
  mutate(AnyNAs = rowSums(is.na(.)) >= 1)
write_xlsx(stats, path= here::here("04_Analysis","plots","paperfigures", "stats", "Figure1", "Figure1_BA1Only.xlsx"))
#####
#####
